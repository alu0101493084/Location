...
def localizacion(balizas, real, ideal, centro, radio, mostrar=False):
  ...
  realMedida = real.senseDistance(objetivos)
  realAngle  = real.senseAngle(objetivos)

  incremento = 0.1

  # Bucle FOR coordenada Y
  for y in np.linspace(centro[1]-radio,centro[1]+radio,2*radio*10):
    fila = []
    # Bucle FOR coordenada X
    for x in np.linspace(centro[0]-radio,centro[0]+radio,2*radio*10):
      # Para cada punto de la rejilla, creamos un robot ideal en esa posición
      ideal.set(x,y,real.orientation)
      # Y calculamos su medida sensorial
      idealMedida = ideal.senseDistance(objetivos)
      idealAngle  = ideal.senseAngle(objetivos)
      # Calculamos el error entre la medida ideal y la real
      error = 0.
      for i in range(len(objetivos)):
        error += (realMedida[i]-idealMedida[i])**2
      error += (realAngle - idealAngle)**2
      fila.append(error)
    imagen.append(fila)
  # Ahora buscamos el mínimo error en la imagen
  minError = float('inf')
  minX     = centro[0]
  minY     = centro[1]
  for iy in range(len(imagen)):
    for ix in range(len(imagen[iy])):
      if imagen[iy][ix] < minError:
        minError = imagen[iy][ix]
        minX     = centro[0]-radio + ix*incremento
        minY     = centro[1]-radio + iy*incremento
  # Fijamos el robot ideal en la posición de mínimo error
  ideal.set(minX,minY,real.orientation)
...
real.set_noise(.05,.05,.1)  # Ruido lineal / radial / de sensado
real.set(*P_INICIAL)
